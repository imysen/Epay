...
<div class="ep-pay-card" x-data="wapPay()" x-init="init()">
  ...
  <div class="ep-qr-area"><div class="ep-qr-box" id="qrcode"></div><div class="ep-scan-hint" style="margin-top:8px"><span class="ch-icon">请使用手机QQ扫描二维码支付</span></div></div>
  <div style="padding:0 20px 12px;display:flex;flex-direction:column;gap:8px">
    <a class="ep-btn ep-btn-primary" style="height:40px;width:100%" href="mqqapi://">打开QQ</a>
  ...
  <div x-data="poller('/getshop.php', {type:'qqpay', trade_no:'<?=addslashes($order['trade_no'])?>'}, {interval:2000, delay:3000})" @poll-ok.window="onOk()" style="display:none"></div>
</div>
...
function wapPay(){
  ...
  return {
    ...
    downloadQR(){var c=$('#qrcode canvas')[0];if(c){var a=document.createElement('a');a.download='QQ二维码.png';a.href=c.toDataURL('image/png');a.click();}},
    async copyLink(){try{await navigator.clipboard.writeText(u);epToast('success','链接已复制');}catch{epToast('info','请长按手动复制');}},
    async checkResult(){try{var d=await this.$fetch('/getshop.php',{method:'GET',body:{type:'qqpay',trade_no:'<?=addslashes($order['trade_no'])?>'}});if(d.code===1){epToast('success','支付成功,正在跳转');setTimeout(function(){window.location.href=d.backurl;},300);}else{epToast('info','您还未完成付款');}}catch(e){epToast('error','服务器错误');}},
