...
<div class="ep-pay-card" x-data="qrcodePay()" x-init="init()">
  ...
  <div class="ep-qr-area">
    ...
    <div class="ep-scan-hint">
      <svg class="ch-icon" width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2L2 7l10 5 10-5-10-5z"/></svg>
      请使用京东APP扫一扫
    </div>
  </div>
  ...
  <div x-data="poller('/getshop.php', {type:'jdpay', trade_no:'<?=addslashes($order['trade_no'])?>'}, {interval:2000, delay:2000})" @poll-ok.window="onOk()" style="display:none"></div>
</div>
...
function qrcodePay(){
  return {
    ...
    renderQR(){var u=<?=json_encode($code_url)?>;if(u.indexOf('data:image/')>-1){$('#qrcode').html('<img src="'+u+'" width="200" height="200"/>');}else{$('#qrcode').qrcode({text:u,width:200,height:200,foreground:'#000000',background:'#ffffff',typeNumber:-1});}},
